...
class NeuralNetwork():
    ...
    def forward_pass(self, X) -> (List[np.ndarray], List[np.ndarray]):
        ...
        # print(Xi)
        Ai = Xi @ self.W[0]
        Zi = self.activations[0].value(Ai)
        A_vals.append(Ai)
        Z_vals.append(Zi)

        for i in range(1, self.n_layers):
            Xi = np.insert(Z_vals[-1], 0, 1, axis=1)
            Ai = Xi @ self.W[i]
            Zi = self.activations[i].value(Ai)
            A_vals.append(Ai)
            Z_vals.append(Zi)
    ...
    def backward_pass(self, A_vals, dLdyhat) -> List[np.ndarray]:
        ...
        delta_n = dLdyhat * self.activations[-1].derivative(A_vals[-1])
        deltas.append(delta_n)
        
        for i in reversed(range(0, self.n_layers-1)):
            tmp0 = self.activations[i].derivative(A_vals[i])
            tmp2 = np.delete(self.W[i+1], (0), axis=0)
            tmp00 = deltas[-1] @ tmp2.T
            delta = tmp00 * tmp0
            deltas.append(delta)
    ...
    def update_weights(self, X, Z_vals, deltas) -> List[np.ndarray]:
        ...
        for i in reversed(range(1, self.n_layers)):
            tmp1 = self.activations[i].value(np.insert(Z_vals[i-1], 0, 1, axis=1))
            dEdWi = deltas[i].T @ tmp1
            # dEdWi = deltas[i] * self.activations[i].derivative(Z_vals[i]) * self.activations[i-1].value(Z_vals[i-1])
            self.W[i] = self.W[i] - self.learning_rate * dEdWi.T
        
        # dEdWi = deltas[0] @ self.activations[0].derivative(Z_vals[0])
        ztmp2 = self.activations[0].value(np.insert(X, 0, 1, axis=1))
        dEdWi = deltas[0].T @ ztmp2
        self.W[0] = self.W[0] - self.learning_rate * dEdWi.T
        return self.W
