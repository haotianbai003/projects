...
class Sigmoid(Activation):
    ...
    def value(self, x: np.ndarray) -> np.ndarray:
        ...
        expon = -1 * self.k * x
        e_vector = np.ones(np.shape(x)) * np.e
        denom = 1 + np.power(e_vector, expon)
        return np.ones(np.shape(denom)) / denom
    ...
    def derivative(self, x: np.ndarray) -> np.ndarray:
        ...
        '''
        val = self.value(x)
        return (val * (np.ones(x.shape) - val))
...
class CrossEntropy(Loss):
    ...
    def value(self, y_hat: np.ndarray, y: np.ndarray) -> float:
        ...
        '''
        array_ones = np.ones(y.shape)
        return np.mean(
            -1 * (y * np.log(y_hat) + (array_ones - y) * np.log(array_ones - y_hat))
        )
    ...
    def derivative(self, y_hat: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        :return dLdy_hat: The derivative of the loss function with respect to each prediction, with shape (n, 1)
        '''
        return (y_hat - y) / (y_hat * (np.ones(y.shape) - y_hat)) / y.shape[0]
...
def accuracy(y_hat: np.ndarray, y: np.ndarray, classification_threshold=0.5) -> float:
    ...
    :return acc: accuracy
    '''
    y_hat_class = (y_hat >= classification_threshold).astype(int)
    y = y.astype(int)
    n = y_hat.shape[0]
    return np.sum(y_hat_class == y) / n
